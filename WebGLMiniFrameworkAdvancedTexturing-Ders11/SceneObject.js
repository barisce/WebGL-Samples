function SceneObject(mesh, material){
    ...
    function drawSceneObjects(camera = window.mainCamera){
        for(var i=0; i<window.sceneObjects.length; i++){
            var sceneObject = window.sceneObjects[i];
            if(camera.ignoredSceneObjects.indexOf(sceneObject) == -1)
                window.sceneObjects[i].render(camera);
        }
    }
